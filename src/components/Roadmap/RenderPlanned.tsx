...
export default function RenderPlanned() {

    const { H3, P1, liStyle, P3 } = tailwind()
    const { planneds, rightCase2, rightCase, commentsCount } = Functions()
    
    return (
        <div className="flex flex-col gap-[24px] w-[350px]">
            <div>
                <h3 className={`${H3}`}>Planned ({planneds?.length})</h3>
                <h5 className={`${P1} text-[#647196]`}>Ideas prioritized for research</h5>
            </div>
            {planneds?.map((e) => {
